...
5. LESSON STRUCTURE
   ...
   - For every fill_blank and short_answer question, provide an "answer" (single word or short phrase for fill_blank, 1-3 sentences for short_answer) and a brief "explanation" of the correct answer. MCQ/true_false answers are derived from option is_correct flags.
...
func (g *Generator) buildTopicSessionPrompt(topic string, tier int, remark string, quizMode string, weakConcepts []string, content string) string {
	modeInstruction := ""
	switch quizMode {
	case "options":
	...
	contentInstruction := "No specific lessons provided — cover the topic generally at the user's tier."
	if content != "" {
		contentInstruction = fmt.Sprintf(`REACHABLE LESSONS (content the user has studied — scope questions to these):
%s
WEIGHTING RULES:
- Focus roughly 70%% of questions on the in-progress/most recent lessons and the weak concepts.
- Roughly 20%% should be spaced retrieval from earlier completed lessons.
- Roughly 10%% may be stretch questions, still within the listed lessons.
- NEVER ask about concepts outside the listed lessons.`, content)
	}
...
- For every fill_blank and short_answer question, include an "answer" field (the correct answer) and an "explanation" field (a brief explanation). Never omit these for free-text questions.
...
`, topic, tier, remark, modeInstruction, weakInstruction, contentInstruction, tierLadder)
}
...
func (g *Generator) buildReviewCardsPrompt(topic string, tier int, content string, questionCount int, existingContext string) string {
	if questionCount <= 0 {
		questionCount = 10
	}
...
- Do NOT generate prompts that are identical or near-identical to the EXISTING REVIEW CARDS listed above.
...
- Prioritise concepts listed in WEAK AREAS above — these have low ease_factor or high lapse counts and need more practice.
- Prioritise concepts that are commonly confused or that appeared in the user's incorrect answers if such data is available.`, topic, tier, content, existingSection, questionCount)
}
...
RULES:
- follow_up: a SINGLE field that (1) gives concise, honest feedback on the user's answer — what they got right and what was imprecise or wrong, without harsh grading — then (2) asks ONE "Why?" / "How does this relate to...?" / "What would happen if...?" question that probes the reasoning behind their answer, grounded in the ORIGINAL QUESTION and their answer. It must NOT reveal the correct answer and must be answerable in 1-3 sentences by someone at their tier. Anchor the follow-up to a prior concept or analogy the user already knows when possible.
- explanation: the correct answer to the ORIGINAL question in general, neutral form (2-4 sentences). NEVER reference the user's answer and NEVER assume it was right or wrong — no "that's right", "correct", "good", "you missed..." or similar. Just the plain model answer.
